            ...
            <ul class="navbar-nav ml-auto">
                ...
                @php
                    ...
                    foreach ($cart as $item) {
                       $groupedCart[$item['vendor_id']][] = $item;
                    }

                    $vendors = App\Models\Vendor::whereIn('id',array_keys($groupedCart))->get()->keyBy('id');
                        ...
                        @foreach ($groupedCart as $vendorId => $items)
                            @if (isset($vendors[$vendorId]))
                                ...
                                <div class="dropdown-cart-top-header p-4">
                                    <img class="img-fluid mr-3" alt="osahan" src="{{ asset('upload/vendor_images/' . $vendor->photo) }}">
                                    <h6 class="mb-0">{{ $vendor->name }}</h6>
                                    <p class="text-secondary mb-0">{{ $vendor->address }}</p>
                                </div>
                            @endif
                        @endforeach
